...
## Commands
Data for dynamic position encoding translation model (check folder examples/dynamic_encoding/)
* Install fast_align in repo
...
```
- Obtain adjusted alignments using adjust_alignments.py and changing parameters within the file

For training a model, use the following command:
```bash
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 fairseq-train data-bin/100_3.5_40k_wmt14_en_de_wordalign \
...
--alignment-lambda 0.5 --label-smoothing 0.1 --weight-decay 0.0 \     --max-tokens  4096  --save-dir ./paper_checkpoints/lambda0.5_2layers_nosum_unified \
...
```bash
python scripts/average_checkpoints.py --inputs examples/dynamic-position/paper_checkpoints/lambda0.5_2layers_nosum_unified \
--num-update-checkpoints 5 --output examples/dynamic-position/paper_checkpoints/lambda0.5_2layers_nosum_unified/average.pt
```
And generate results
```bash
CUDA_VISIBLE_DEVICES=0 fairseq-generate  data-bin/100_3.5_40k_wmt14_en_de_wordalign \
--path paper_checkpoints/lambda0.5_2layers_nosum_unified/average.pt \
